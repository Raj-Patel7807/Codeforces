...
void solve(ll tt) {
    IN(ll, n); IN(string, s); debug(n, s);
    string a = "", b = ""; bool ok = false;
    FOR(n) {
        if(s[i] == '0') { a += '0'; b += '0'; }
        else if(s[i] == '1' && !ok) { a += '1'; b += '0'; ok = true; }
        else if(s[i] == '1' && ok) { a += '0'; b += '1'; }
        else if(s[i] == '2' && !ok) { a += '1'; b += '1'; }
        else if(s[i] == '2' && ok) { a += '0'; b += '2'; }
        debug(a, b);
    }
    OUT(a, b);
}
