...
void solve(ll tt) {
    IN(ll, n); vc<pr<ll, ll>> a(n+1); a[0] = {0, 0}; debug(tt, n);
    FOR(i, 1, n+1) cin >> a[i].FF >> a[i].SS; debug(a);
    FOR(i, 1, n+1) if(a[i].FF - a[i-1].FF < 0 || a[i].SS - a[i-1].SS < 0 || a[i].FF - a[i-1].FF < a[i].SS - a[i-1].SS) { NO(); return; }
    YES();
}
