...
void solve(ll tt) {
    IN(ll, n, k); deque<ll> a(n);
    FOR(n) cin >> a[i]; debug(tt, n, k, a);
    while(LEN(a) > 1 && k) {
        ll mn = min(a.front(), a.back()); debug(mn);
        if(k < 2*mn) {
            a.front() -= k/2 + (k & 1);
            a.back() -= k/2; k = 0;
        } else {
            a.front() -= mn; a.back() -= mn;
            k -= 2*mn;
        }
        if(!a.front()) a.pop_front();
        if(!a.back()) a.POP(); debug(a, k);
    }
    ll ans = n - LEN(a); if(LEN(a) && a.front() <= k) ans++;
    OUT(ans);
}
