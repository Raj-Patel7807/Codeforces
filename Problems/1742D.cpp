...
map<ll, vc<ll>> mp;
void fun(map<ll, vc<ll>>& mp) {
    FOR(i, 1, 1001) FOR(j, 1, 1001) if(gcd(i, j) == 1) mp[i].PB(j);
}
...
inline void solve(ll tt) {
    ...
    map<ll, vc<ll>> mpp;
    FOR(n) mpp[a[i]].PB(i+1);

    ll ans = -1;
    FOR(i, 1, 1001) FORE(j, mp[i]) if(!mpp[i].empty() && !mpp[j].empty()) {
        ans = max(ans, mpp[i].back() + mpp[j].back());
    }
