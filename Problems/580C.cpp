...
void DFStree(ll node, ll parent, vector<vector<ll>> &tree, vc<ll>& a, ll& ans, ll cat, ll m) {
    if(a[node]) cat++;
    else cat = 0;
    if(cat > m) return;
    bool leaf = true;
    for(ll child : tree[node]) {
        if(child != parent) { leaf = false; DFStree(child, node, tree, a, ans, cat, m); }
    }
    if(leaf) ans++;
}
