...
void solve(ll tt) {
    ...
    FOR(i, 26) if(s.find((char)('a'+i)) == string::npos) { OUT((char)('a'+i)); return; }
    FOR(i, 26) FOR(j, 26) {
        string ns = ""; ns += (char)('a'+i); ns += (char)('a'+j); debug(ns);
        if(s.find(ns) == string::npos) { OUT(ns); return; }
    }
    FOR(i, 26) FOR(j, 26) FOR(k, 26) {
        string ns = ""; ns += (char)('a'+i); ns += (char)('a'+j); ns += (char)('a'+k); debug(ns);
        if(s.find(ns) == string::npos) { OUT(ns); return; }
    }
}
