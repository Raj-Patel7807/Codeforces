...
inline void solve(ll tt) {
    ...
    ll x = INF;
    FOR(k) {
        ll y = abs(v[a-1].FF - v[i].FF) + abs(v[a-1].SS - v[i].SS);
        if(x > y) {
            x = y;
            mna.FF = v[i].FF; mna.SS = v[i].SS;
        }
    }
    x = INF;
    FOR(k) {
        ll y = abs(v[b-1].FF - v[i].FF) + abs(v[b-1].SS - v[i].SS);
        if(x > y) {
            x = y;
            mnb.FF = v[i].FF; mnb.SS = v[i].SS;
        }
    }
    debug(mna, mnb);

    ll ans = abs(mna.FF - v[a-1].FF) + abs(mna.SS - v[a-1].SS);
    ans += abs(mnb.FF - v[b-1].FF) + abs(mnb.SS - v[b-1].SS);

    ll ans1 = abs(v[a-1].FF - v[b-1].FF) + abs(v[a-1].SS - v[b-1].SS);
