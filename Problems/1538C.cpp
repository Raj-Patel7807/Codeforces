...
void solve(ll tt) {
    IN(ll, n, l, r); VIN(ll, a, n); debug(n, l, r, a);
    sort(all(a)); debug(a);
    ll cnt = 0, ct = 0, cn = 0;
    FOR(n) cnt += UB(all(a), r-a[i]) - a.begin(); debug(cnt);
    FOR(n) ct += LB(all(a), l-a[i]) - a.begin(); debug(ct);
    FOR(n) if(l <= 2*a[i] && r >= 2*a[i]) cn++; debug(cn);
    OUT((cnt - ct - cn)/2);
}
