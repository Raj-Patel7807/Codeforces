...
void solve(ll tt) {
    ...
    vc<pr<char, ll>> vp;
    FOR(n) {
        if(vp.empty() || vp.back().FF != s[i]) vp.PUSH({s[i], 1});
        else vp.back().SS++;
    }
    FOR(i, 1, LEN(vp)-1) if(vp[i-1].FF != vp[i+1].FF) ans = min(ans, vp[i].SS+2);
    OUT((ans ^ MOD ? ans : 0));

    // ll on = count(all(s), '1'), to = count(all(s), '2'), th = n-on-to;
    // if(!on || !to || !th) { OUT(0); return; }

    // ll ans = n;
    // FOR(i, 1, n) {
    //     if(s[i] != s[i-1]) {
    //         uset<char> st; ll j = i-1;
    //         while(j < n && LEN(st) < 3) st.insert(s[j++]); debug(i, j, st);
    //         if(!(LEN(st) ^ 3)) {
    //             ans = min(ans, j-i+1);
    //             if(ans == 3) { OUT(ans); return; }
    //         }
    //     }
    // }
    // OUT(ans);
