...
inline void solve(ll tt) {
    ...
    ll ans = 0;
    ans = max(ans, (v[0][len(v[0])-1] - v[0][0])*h);
    ans = max(ans, (v[1][len(v[1])-1] - v[1][0])*h);
    ans = max(ans, (v[2][len(v[2])-1] - v[2][0])*w);
    ans = max(ans, (v[3][len(v[3])-1] - v[3][0])*w);
