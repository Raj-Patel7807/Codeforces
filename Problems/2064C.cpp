...
void solve(ll tt) {
    IN(ll, n); VIN(ll, a, n); debug(a, n);
    vc<ll> pre(n, 0), suf(n, 0); ll ans = 0;
    if(a[0] > 0) pre[0] = a[0]; if(a[n-1] < 0) suf[n-1] = -a[n-1];
    FOR(i, 1, n) { pre[i] = pre[i-1]; if(a[i] > 0) pre[i] += a[i]; } debug(pre);
    FORR(n-1) { suf[i] = suf[i+1]; if(a[i] < 0) suf[i] -= a[i]; } debug(suf);
    FOR(n) ans = max(ans, pre[i]+suf[i]); OUT(ans);
}
