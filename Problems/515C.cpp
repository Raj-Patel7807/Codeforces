...
void solve(ll tt) {
    IN(ll, n); IN(string, s); string ans = ""; debug(n, s, ans);
    ll fr = count(all(s), '4'), ei = count(all(s), '8'), sx = count(all(s), '6'), ni = count(all(s), '9');
    FOR(n) if(s[i] != '1' && s[i] != '0' && s[i] != '4' && s[i] != '8' && s[i] != '6' && s[i] != '9') ans += s[i];
    FOR(fr) ans += "322"; FOR(ei) ans += "7222"; FOR(sx) ans += "53"; FOR(ni) ans += "7332";
    sort(rall(ans)); OUT(ans);
}
