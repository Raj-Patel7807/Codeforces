...
void solve(ll tt) {
    IN(string, s, t); ll n = LEN(s); debug(n, s, t);
    ll ps = count(all(s), '+'), ns = n-ps; debug(ps, ns);
    ll pt = count(all(t), '+'), nt = count(all(t), '-'), qt = n-pt-nt; debug(pt, nt, qt);

    cout << fixed << setprecision(12);
    if(!qt && ps == pt && ns == nt) { OUT(1.00); return; }
    if(!qt && (ps != pt || ns != nt)) { OUT(0.00); return; }
    if(qt && (!ps && pt || !ns && nt)) { OUT(0.00); return; }

    ll tp = ps - pt, tn = ns - nt; debug(tp, tn);
    OUT((factorial(qt) / (factorial(tp) * factorial(tn))) / pow(2, qt));
}
