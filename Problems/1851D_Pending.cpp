...
void solve(ll tt) {
    IN(ll, n); VIN(ll, a, n-1); debug(n, a);
    if(a[n-2] > n*(n+1)/2) { NO(); return; }
    map<ll, ll> mp; mp[1]++; bool ok = false;
    FOR(i, 1, n-1) {
        mp[a[i] - a[i-1]]++;
        if(a[i] - a[i-1] > n && !ok) ok = true;
        else if(a[i] - a[i-1] > n && ok) { NO(); return; }
    }
    FORE(i, mp) if(i.SS > 1) { NO(); return; }
    YES();
}
