...
void solve() {
    ...
    ll r = count(all(s), "rock");
    ll p = count(all(s), "paper");
    ll sc = count(all(s), "scissors");

    if(r == 3 || p == 3 || sc == 3) {
        out('?');
    } else if(r == 2 && sc == 1 || sc == 2 && p == 1 || p == 2 && r == 1) {
        out('?');
    } else if(r == 1 && p == 1) {
        out('?');
    } else {
