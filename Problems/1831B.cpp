...
void solve() {
    ...
    fo(i, 1, n-1) {
        ...
        if(b[i] == b[i-1]) {
            ...
            mp2[b[i-1]] = max(mp2[b[i-1]], cur2);
            cur2 = 1;
        }
    }

    mp1[a[n-1]] = max(mp1[a[n-1]], cur1);
    mp2[b[n-1]] = max(mp2[b[n-1]], cur2);

    ft(i, mp1) {
        mp[i.FF] = mp1[i.FF] + mp2[i.FF];
    }
    ft(i, mp2) {
        mp[i.FF] = mp1[i.FF] + mp2[i.FF];
    }
    ...
    //     }
    // }
    // while (i < len(a)) c[l++] = a[i++];
    // while (j < len(b)) c[l++] = b[j++];
