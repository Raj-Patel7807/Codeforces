...
void solve(ll tt) {
    IN(ll, n, f, k); VIN(ll, a, n); debug(n, f, k, a);
    ll fav = a[f-1]; bool ok = false, okk = false;
    sort(rall(a)); debug(a);
    FOR(k) if(!(a[i] ^ fav)) { ok = true; break; }
    FOR(i, k, n) if(!(a[i] ^ fav)) { okk = true; break; }
    if(ok && okk) { OUT("MAYBE"); return; }
    YES(ok);
}
