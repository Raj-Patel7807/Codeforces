...
void solve(ll tt) {
    ...
    auto check = [&](string t) -> bool{
        if(LEN(t) < 1 || LEN(t) > 16) return false;
        FOR(LEN(t)) if(!isalnum(t[i]) && t[i] != '_') return false; return true;
    };

    ll ida = s.find('@'), idb = s.find('/'); debug(ida, idb);

    if(ida == -1) { NO(); return; }
    if(!check(s.substr(0, ida))) { NO(); return; }

    string hn = "";
    if(idb == -1) hn = s.substr(ida+1);
    else hn = s.substr(ida+1, idb-ida-1); debug(hn);
    if(LEN(hn) < 1 || LEN(hn) > 32) { NO(); return; }

    ll i = 0, j = MOD;
    while(j != -1) {
        j = hn.find('.', i); debug(j, i);
        string hna = "";
        if(j == -1) hna = hn.substr(i);
        else hna = hn.substr(i, j-i); debug(hna);
        if(!check(hna)) { NO(); return; }
        i = j+1;
    }

    if(idb != -1 && !check(s.substr(idb+1))) { NO(); return; }
    YES();
}
