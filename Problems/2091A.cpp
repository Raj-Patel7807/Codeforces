...
void solve(ll tt) {
    ...
    map<ll, pr<ll, ll>> mp; FOR(n) {mp[a[i]].FF++; mp[a[i]].SS++; } debug(mp);

    if(mp[0].FF < 3 || mp[1].FF < 1 || mp[2].FF < 2 || mp[3].FF < 1 || mp[5].FF < 1) {
        OUT(0); return;
    } debug("Hye");
    FOR(n) {
        if(mp[a[i]].FF > 0) mp[a[i]].FF--; debug(a[i], mp);
        if(mp[0].FF <= mp[0].SS-3 && mp[1].FF <= mp[1].SS-1 && mp[2].FF <= mp[2].SS-2 && mp[3].FF <= mp[3].SS-1 && mp[5].FF <= mp[5].SS-1) { OUT(i+1); return; }
    }
}
