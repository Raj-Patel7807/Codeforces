...
void solve(ll tt) {
    ...
    if(!cnt) { OUT(1); OUTT(1, n); ln; return; }
    if(cnt == n) { OUT(3); OUTT(1, 2); ln; OUTT(2, n-1); ln; OUTT(1, 2); ln; return; }
    if(a[0]) { OUT(2); OUTT(2, n); ln; OUTT(1, 2); ln; return; }
    if(a[n-1]) { OUT(2); OUTT(1, n-1); ln; OUTT(1, 2); ln; return; }
    ll id = -1; FOR(n) if(a[i]) { id = i+1; break; }
    if(id == 2) { OUT(3); OUTT(1, id); ln; OUTT(id, n-1); ln; OUTT(1, 2); ln; return; }
    OUT(3); OUTT(1, id-1); ln; OUTT(2, n-id+2); ln; OUTT(1, 2); ln; return;
}
