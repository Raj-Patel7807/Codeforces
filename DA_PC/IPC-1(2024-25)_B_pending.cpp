...
/*
    @Raj_Patel_7807
    Code By : Raj_Patel
    Date : 25/01/2025    Time -> 11:29:31
    Problem : IPC-1(2024-25)_B
    Link : https://codeforces.com/gym/551595
*/
...
inline void solve(ll tt) {
    ...
    fo(i, 0, k-1) {
        fo(j, 0, l-1) {
            if(v1[i].SS == v2[i].SS) {
                ans = max(ans, v1[i].SS * v2[i].SS);
            }
            if(m-v1[i].SS == n-v2[i].SS) {
                ans = max(ans, (m-v1[i].SS) * (n-v2[i].SS));
            }
        }
    }
