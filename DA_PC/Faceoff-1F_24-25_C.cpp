...
void solve(ll tt) {
    IN(ll, n); VIN(ll, a, n); debug(n, a);
    vc<ll> pre(n, 0); pre[0] = a[0]; FOR(i, 1, n) pre[i] = pre[i-1] + a[i]; debug(pre);
    ll mnd = MOD, mnid = -1, sum = accumulate(all(a), 0LL);
    FOR(n) {
        if(abs(sum - 2*pre[i]) < mnd) {
            mnd = abs(sum - 2*pre[i]); debug(mnd);
            mnid = i+1;
        }
    } OUTT(mnid, mnd); ln;
}
