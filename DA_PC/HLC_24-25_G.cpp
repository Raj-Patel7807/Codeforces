...
constexpr ld E = 2.7182818284590452353602L;
...
void solve(ll tt) {
    ...
    vc<string> ss(n, ""); ll mxbit = 0;
    FOR(i, n) {
        ll nm = a[i];
        if(!nm) ss[i] += "0";
        while(nm) { ss[i] += (char)(nm % 2 + '0'); nm >>= 1; }
        reverse(all(ss[i]));
        FORR(j, LEN(ss[i])-2, 0) {
            if(ss[i][j] == ss[i][j+1] && ss[i][j+1] == ss[i][j+2]) {
                ss[i].erase(ss[i].begin()+j, ss[i].begin()+j+3);
            }
        }
        mxbit = max(mxbit, LEN(ss[i]));
    }
    FOR(i, n) if(LEN(ss[i]) < mxbit) ss[i] = string(mxbit - LEN(ss[i]), '0') + ss[i];
    debug(ss, mxbit);

    ll ans = 0;
    FOR(i, mxbit) {
        ll cnt = 0; FORR(j, n) cnt += ss[j][i] - '0';
        ans = (ans << 1) | (cnt > n/2);
    }
    OUT(ans);
}
