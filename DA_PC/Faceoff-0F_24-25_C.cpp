...
void solve(ll tt) {
    IN(ll, n, m); VVIN(ll, a, n, m); debug(n, m, a);
    vc<vc<ll>> ans(n, vc<ll>(m, 0));

    FOR(i, n) FOR(j, m) if(a[i][j] > 0) {
        FOR(k, max(i-5, 0LL), min(i+5, n)) FOR(l, max(j-5, 0LL), min(j+5, m)) {
            ans[k][l] += max(0LL, a[i][j] - abs(k - i) - abs(l - j));
        }
    }
    FOR(i, n) { FOR(j, m) OUTT(ans[i][j]); ln; }
}
